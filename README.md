...
Каждый проект отображает свой статус и общее затраченное время. Управление таймером теперь использует `ProjectRepository`, поэтому информация о времени сохраняется в базу данных.
